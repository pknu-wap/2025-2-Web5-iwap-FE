...
const plots: Record<string, { data: any[]; title:string }> = {
  ...
  "2": { 
    data: [{ x: xVals, y: fVals, type: "scatter", mode: "lines", line: { color: 'red' } }], // <-- 변경: 선 색상을 'red'로 지정
  ...
  "3": { 
    data: [{ 
      ...
      z: zGrid, 
      type: "surface",
      // <-- 변경: colorscale로 그라데이션 지정
      // 0(낮은 값)은 어두운 회색, 1(높은 값)은 채도 낮은 빨간색
  ...
  "5": { 
    data: [{ 
      ...
      y: yHeart, 
      mode: "lines+markers", 
      // <-- 변경: 동적 heartColors 대신 마커와 선 모두 'red'로 고정
        ...
        {width && height && (
          <Plot
            ...
            layout={{
              ...
              yaxis: {
                title: "Y",
                scaleanchor: "x", // y축의 스케일을 x축에 고정
                scaleratio: 1,    // x축과 y축의 비율을 1:1로 설정
              },
            }}
