/**
 * @file /api/piano/midi/[task_id] 경로의 GET 요청을 백엔드로 전달하는 API 프록시.
...
export async function GET(request, { params }) {
  ...
  console.log(`[API/piano/midi] GET request started: task_id=${task_id}`);

  if (!BACKEND_URL) {
    console.error('[API/piano/midi] Error: ASYNC_BACKEND_API_URL environment variable is not set.');
  ...
  try {
    const targetUrl = `${BACKEND_URL}/api/piano/midi/${task_id}`;
    ...
    console.log(`[API/piano/midi] Backend response: ${response.status} ${response.statusText}`);
    ...
    console.error(`[API/piano/midi] Proxy error: ${error.message}`);
