...
export default function AsciiArtDisplay({
  ...
  const bind = useDrag(({ down, xy: [x], tap, event }) => { if (isProcessing || !sliderBarRef.current) return; if (tap) event.preventDefault(); const { left, width } = sliderBarRef.current.getBoundingClientRect(); const progress = Math.max(0, Math.min(1, (x - left) / width)); const rawValue = minRes + progress * (maxRes - minRes); const steppedValue = Math.round(rawValue / 1) * 1; const finalValue = Math.max(minRes, Math.min(maxRes, steppedValue)); setResolution(finalValue); if (!down) { if (finalValue !== asciiResult.initialResolution) { onResolutionChange(finalValue); } } }, { filterTaps: true });
        ...
        <div className="relative w-full max-w-5xl mx-auto md:flex md:items-start md:justify-center md:gap-1">
          <div className="flex flex-col items-center w-full">
            ...
            <div className="bg-white p-1.5 shadow-lg w-full max-w-[620px]">
              <div ref={containerRef} className="w-full aspect-square flex items-center justify-center overflow-hidden">
                <div ref={zoomWrapperRef} className="bg-black overflow-hidden">
                  {/* [수정] 텍스트 선택 방지 스타일 추가 및 직접 렌더링 로직 */}
            ...
            <div className="w-full max-w-[620px] mt-8">
              ...
              <div ref={sliderBarRef} {...bind()} className={`relative flex-1 h-8 flex items-center touch-none ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                <div className="relative w-full h-1.5">
                  ...
                  <div className="absolute top-1/2 w-1 h-4 bg-white pointer-events-none z-10 rounded-full" style={{ left: `${progressPercent}%`, transform: `translateX(-50%) translateY(-50%)` }} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="absolute top-[88px] right-4 z-20 md:relative md:top-auto md:right-auto md:mt-[72px] transform-origin-top-right transition-transform duration-200 ease-in-out scale-75 md-scale-100">
            <div className="w-72 h-auto p-4 bg-white/40 border border-white rounded-md flex flex-col gap-4">
              <div className="flex items-center justify-around">
                {PNG_SCALE_OPTIONS.map(opt => <label key={opt} className="flex items-center gap-2 cursor-pointer"><div className="relative w-3.5 h-3.5"><input type="radio" name="png-scale" value={opt} checked={pngScaleFactor === opt} onChange={(e) => setPngScaleFactor(Number(e.target.value))} className="peer sr-only" disabled={isProcessing} /><div className="w-full h-full bg-zinc-300 rounded-full border border-zinc-400 peer-checked:bg-white"></div><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1.2 w-2 h-2 rounded-full bg-zinc-300 peer-checked:bg-white"></div></div><span className="text-white text-xl font-semibold">{opt}x</span></label>)}
              </div>
